      ...
      <div className="w-full z-10 relative px-2">
        ...
        >
          ...
          {Object.keys(activeWidgets).map(key => {
            ...
            const boxProps = {
                ...
                title: isExtra ? `${type}.exe (${key.split('-')[1].slice(-4)})` : (
                    ['snake', 'life', 'fireworks', 'starfield', 'rain', 'maze', 'pipes', 'matrix', 'donut', 'market'].includes(type)
                        ...
                        : (type === 'todo' ? 'note.txt' : type === 'search' ? 'search' : type === 'links' ? 'links.href' : type === 'weather' ? 'forecast.py' : type === 'datetime' ? 'clock.sys' : type)
                ),
            ...
            switch (type) {
                case 'search':
                  return <TuiBox {...boxProps} title="search"><SearchWidget /></TuiBox>;
                case 'datetime':
                ...
                case 'todo':
                    return <TuiBox {...boxProps} title="note.txt"><TodoWidget tasks={todos} setTasks={setTodos} todoistConfig={todoistConfig} /></TuiBox>;
                case 'links':
